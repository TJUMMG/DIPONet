...
from model.vgg_aspp import VGG
...
class GOFM(nn.Module):
    def __init__(self, in_channel, out_channel, nums, conv_size, layer):
        ...
        self.conv_res = nn.Conv2d(in_channel, out_channel, 3, padding=1)
...
class FM(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        ...
        if self.layer == 5:
            ...
            x3 = self.conv3(x3)
            x3 = F.interpolate(x3, scale_factor=2**(5-self.layer))
            z3 = self.relu(xf1 * x3)
...
class DSIO(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4):
        if self.layer == 5:
            ...
            x1 = F.interpolate(x1, scale_factor=1/8)
...
"""
    DIPONet_VGG: This is our proposed model based on VGG16.
...
"""
class DIPONet_VGG(nn.Module):
    def __init__(self, pretrained):
        super(DIPONet_VGG, self).__init__()
        self.vgg = VGG(pretrained)
        self.ref5 = GOFM(512, 128, 4, 4, 5)
        self.ref4 = GOFM(512, 128, 4, 3, 4)
        self.ref3 = GOFM(256, 128, 2, 2, 3)
        self.ref2 = GOFM(128, 128, 1, 1, 2)
        self.ref_edge = GOFM(128, 128, 1, 1, 1)
        ...
        self.spvedge1 = Spv2(128, 2)
        self.spvsal1 = Spv2(128, 16)
        self.spvedge = Spv(128)
        self.spvsal = Spv(128)
    ...
    def forward(self, x):
        ...
        edge128 = self.ref_edge(x2, xf2)
        spvedge1 = self.spvedge1(edge128)

        edge5, xf5 = self.ca5(edge128, xf5, xf5, x6)
        edge4, xf4 = self.ca4(edge5, xf4, xf5, x6)
        edge3, xf3 = self.ca3(edge4, xf3, xf4, xf5)
        edge2, xf2 = self.ca2(edge3, xf2, xf3, xf5)

        spvedge2 = self.spvedge(edge2)
        globalsal = self.spvsal(xf2)

        return spvedge1, globalsal1, spvedge2, globalsal
